...
export async function parseConfig(file: Express.Multer.File, req: Request) {
  ...
  const DOCUMENT_TO_CONFIG_PROMPT = [
    ...
    "",
    "DOMAIN FOCUS: Insurance and financial services compliance only. If the uploaded document is not related to insurance, finance, or employee compliance in these sectors, still do your best to map it to the closest compliance context.",
    "",
    "Your task:",
    ...
    "  - dashboard: title (e.g. 'AML Compliance Dashboard'), company (extracted from document or 'Allied Financial Services').",
    "",
