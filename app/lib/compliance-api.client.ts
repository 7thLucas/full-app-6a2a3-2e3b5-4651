...
function getTargetDepartment(config: JudgmentConfig): Department {
  ...
  return target === "Sales" || target === "Compliance" || target === "All" ? target : "All";
}
...
function buildDirectConfig(input: {
  ...
  const requirements = input.requirements
    ...
    .map((item) => item.trim())
    .filter(Boolean);
  const pluginId = `insurance-${slugify(input.title)}-${Date.now()}`;
  ...
  return {
    ...
    variables: {
      ...
      dashboard: {
        title: "Insurance Compliance Dashboard",
        company: "Allied Financial Services",
      },
    },
  };
}
...
function enrichParsedConfig(config: JudgmentConfig, input: {
  ...
  return {
    ...config,
    pluginId: config.pluginId || `insurance-${slugify(config.name)}-${Date.now()}`,
    variables: {
      ...(config.variables || {}),
      targetDepartment: input.targetDepartment,
      configType: input.type,
...
function mapSubmission(submission: JudgmentSubmission, configTitle = ""): ComplianceSubmissionDto {
  ...
  return {
    ...
    department: inputData.department === "Compliance" ? "Compliance" : "Sales",
...
export async function createComplianceSubmission(input: {
  ...
  // No quizScore: the employee form only collects an evidence note + a file.
  const { file, ...inputData } = input;
  const form = new FormData();
  form.set("inputData", JSON.stringify(inputData));
  // Backend route mounts `upload.array("files")`, so the field MUST be "files".
  // Sending it as "file" triggers MulterError: Unexpected field → 500.
